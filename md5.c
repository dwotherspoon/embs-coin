...
void md5_setup(MD5_ctx *  ctx, uint32_t start, uint8_t diff, uint32_t msg[15]) {
	uint8_t i;
	ctx->diff = diff;
	ctx->msg[0] = start;
	for (i = 0; i < 15; i++) {
...
/* 
Returns leading zeros in MD5 hash.
Based on: http://www.nayuki.io/page/fast-md5-hash-implementation-in-x86-assembly
*/
uint8_t md5_hash(MD5_ctx * ctx) {
	...
	a += (expr) + UINT32_C(t) + ctx->msg[offset+k];  \
	a = b + (a << s | a >> (32 - s));
	...
	h0 = ENDSWAP(h0);
	return leading_zeros(h0);
}

static inline uint8_t leading_zeros(uint32_t val) {
	uint8_t i;
	for (i = 0; i < 32; i++) {
		if (val & 0x80000000 >> i) break; 
	}
	return i;
}
